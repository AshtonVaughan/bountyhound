...
class CacheDetector:
    ...
    def _responses_semantically_equal(self, text1: str, text2: str) -> bool:
        """Compare responses ignoring dynamic fields."""
        dynamic_patterns = [
            r'csrf[_-]?token["\s:=]+["\']?[\w-]+',
            r'nonce["\s:=]+["\']?[\w-]+',
        ...
        for pattern in dynamic_patterns:
            clean1 = re.sub(pattern, '[DYNAMIC]', clean1, flags=re.IGNORECASE)
            clean2 = re.sub(pattern, '[DYNAMIC]', clean2, flags=re.IGNORECASE)
        return clean1 == clean2

    def estimate_cache_ttl(self, url: str, samples: int = 5) -> float | None:
        """Estimate cache TTL from Age header progression."""
        ages = []
        ...
        if len(ages) >= 2:
            # Age should increase by ~1 per second
            return max(ages) - min(ages) + max(ages)  # Rough TTL estimate
        return None
    ...
    def _test_cache_behavior(self) -> bool:
        ...
        try:
            ...
            # Check if responses are semantically identical (ignoring dynamic fields)
            if self._responses_semantically_equal(response1.text, response2.text):
...
class UnkeyedInputTester:
    ...
    async def _test_cache_poison_concurrent(self, url: str, header: str, poison_value: str) -> bool:
        """Test cache poisoning by firing poisoned + clean requests concurrently."""
        import asyncio

        cache_buster = f"?_cp={int(time.time())}"
        test_url = url + cache_buster

        try:
            import aiohttp
            async with aiohttp.ClientSession() as session:
                # Fire poisoned request and clean request at the same time
                poisoned = session.get(test_url, headers={header: poison_value}, ssl=False)
                clean = session.get(test_url, ssl=False)
                responses = await asyncio.gather(poisoned, clean, return_exceptions=True)

            if any(isinstance(r, Exception) for r in responses):
                return False

            # Check if clean response got the poisoned content
            clean_body = await responses[1].text() if hasattr(responses[1], 'text') else ""
            return poison_value in clean_body
        except ImportError:
            # aiohttp not available - fall back to sequential requests
            try:
                resp1 = requests.get(test_url, headers={header: poison_value}, timeout=self.timeout, verify=False)
                resp2 = requests.get(test_url, timeout=self.timeout, verify=False)
                return poison_value in resp2.text
            except Exception:
                return False
        except Exception:
            return False
