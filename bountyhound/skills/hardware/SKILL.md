...
## Phase 0: Device Profiling and Attack Routing
...
| Network appliance (NAS, printer) | Web admin, SMB/FTP/IPP services | Web vulns, default creds, exposed services |

**Step 2: Test in this order (highest ROI first)**

1. **Web interface** (if exists) - test like any web app, but IoT web apps are notoriously weak. Load `injection-attacks` and `auth-attacks` skills.
2. **Default credentials** - check against device model database below
3. **Network services** - nmap scan, test each exposed service
4. **Firmware extraction** - download from vendor, extract secrets
5. **Physical interfaces** - UART/JTAG/SPI (requires physical access)
6. **Wireless protocols** - MQTT/CoAP/BLE/Zigbee (requires radio hardware)

**Step 3: Quick wins checklist (test these first, 10 minutes)**

- [ ] Try admin:admin, admin:password, root:root on web interface
- [ ] Check for firmware update download on vendor website
- [ ] Run `nmap -sV -p- DEVICE_IP` - what's open?
- [ ] Check if UPnP is enabled (`upnpc -l`)
- [ ] Try RTSP at `rtsp://DEVICE_IP:554/` (cameras)
- [ ] Check for exposed API at common paths (/api, /cgi-bin, /goform)
...
**Credential testing workflow (not a list - a sequence):**

1. Try web interface login with: admin/admin, admin/password, admin/[blank], root/root
2. If device model known: search `https://www.defaultpassword.com` and `https://cirt.net/passwords`
3. If SSH/Telnet open: try same credentials + root/[blank], user/user
4. If SNMP open: try community strings "public", "private", device model name
5. If all fail: extract firmware, grep for hardcoded passwords (see Firmware Analysis section)
6. If no credentials needed: some devices have unauthenticated admin access - try accessing /admin, /management, /setup directly

```
REFERENCE - common credential patterns:
