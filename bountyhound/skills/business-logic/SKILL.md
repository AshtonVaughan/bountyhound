...
description: |
  ...
  bypass, privilege escalation, multi-tenancy confusion, coupon/discount abuse, free tier
  exploitation, quota bypass, payment bypass, financial rounding exploitation, deep state
  machine mapping, and cross-microservice logic flaws. Invoke this skill PROACTIVELY whenever:
  testing ANY application with payment/checkout flows, subscription management, user role
  systems, multi-tenant architectures, coupon/discount systems, free trial offerings, API
  rate limits or quotas, order/workflow state machines, or microservice architectures.
  Business logic bugs are the highest-ROI findings because scanners never find them and they
  are always application-specific. If the target has ANY financial feature, this skill applies.
  Use PROACTIVELY - do not wait for a specific hypothesis type to trigger this.
---
...
## Advanced Attack Classes

For deeper business logic exploitation, load the advanced patterns reference:

**Read `references/advanced-patterns.md` when:**
- Target has currency/wallet features (Attack Class 10: Financial Rounding)
- Target has complex multi-step workflows (Attack Class 11: Deep State Machines)
- Target uses microservice architecture (Attack Class 12: Cross-Microservice Logic)

These patterns require more time and expertise but produce higher-severity findings.
