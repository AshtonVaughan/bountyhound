"""Entry point — runs FastAPI + background cleanup for metasploit-claude."""
...
log = logging.getLogger("metasploit-claude")
...
async def run_api() -> None:
    ...
    log.info(f"metasploit-claude API listening on http://127.0.0.1:{API_PORT}")
    await server.serve()
...
async def main() -> None:
    """Run all metasploit-claude services concurrently."""
    log.info("Starting metasploit-claude...")

    if MCP_MODE:
        from mcp_server import MetasploitMCP
        mcp = MetasploitMCP(api_base_url=f"http://127.0.0.1:{API_PORT}")
        mcp.run()
    else:
        tasks = [
        ...
        try:
            ...
            log.info("Shutting down metasploit-claude...")
            for t in tasks:
                t.cancel()
