"""Entry point — runs FastAPI + background cleanup for zeek-claude."""
...
log = logging.getLogger("zeek-claude")

API_PORT = int(os.environ.get("API_PORT", "8198"))
...
async def run_api() -> None:
    ...
    log.info(f"zeek-claude API listening on http://127.0.0.1:{API_PORT}")
    await server.serve()
...
async def main() -> None:
    log.info("Starting zeek-claude...")

    if MCP_MODE:
        from mcp_server import ZeekMCP
        mcp = ZeekMCP(api_base_url=f"http://127.0.0.1:{API_PORT}")
        mcp.run()
    else:
        tasks = [
        ...
        try:
            ...
            log.info("Shutting down zeek-claude...")
            for t in tasks:
                t.cancel()
