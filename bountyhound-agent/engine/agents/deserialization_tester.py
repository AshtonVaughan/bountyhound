"""
Deserialization Tester Agent

Comprehensive Insecure Deserialization (Deserialization) testing agent.
...
class DeserializationTest:
    """Represents a single Deserialization test."""
    name: str
    payload: str
    category: str
...
class DeserializationFinding:
    """Represents an Deserialization vulnerability finding."""
    severity: str
    title: str
    category: str
    payload: str
...
class DeserializationTester:
    """
    Comprehensive Deserialization vulnerability tester.

    Tests for Deserialization vulnerabilities using multiple techniques:
    ...
    def __init__(self, target_url: str, param_name: Optional[str] = None,
        ...
        Args:
            ...
            oast_domain: Out-of-band (OAST) domain for blind Deserialization detection
        """
        ...
        # Track timing baselines for blind Deserialization
        self.timing_baseline: Optional[float] = None

    def run_all_tests(self) -> List[DeserializationFinding]:
        """
        ...
        context = DatabaseHooks.before_test(self.target, 'deserialization_tester')
        ...
        print(f"{Fore.CYAN}[*] Starting comprehensive Deserialization testing...{Style.RESET_ALL}")
        ...
        self._test_blind_deserialization()
        ...
        print(f"\n{Fore.CYAN}=== Deserialization TESTING COMPLETE ==={Style.RESET_ALL}")
    ...
    def _establish_timing_baseline(self):
        """Establish timing baseline for blind Deserialization detection."""
        try:
            start = time.time()
        ...
        for cloud_provider, endpoints in self.METADATA_ENDPOINTS.items():
            for endpoint in endpoints:
                ...
                test = DeserializationTest(
                    ...
                    severity="CRITICAL",
                    description=f"Deserialization to {cloud_provider} metadata service",
    ...
    def _test_metadata_bypasses(self):
        ...
        for name, payload in bypasses:
            ...
            test = DeserializationTest(
                ...
                severity="CRITICAL",
                description=f"Deserialization to AWS metadata using {name}",
                detection_method="response_content"
            )
        ...
        for target in self.INTERNAL_TARGETS:
            ...
            for port in [80, 443, 22, 3306, 5432, 6379, 27017]:
                ...
                test = DeserializationTest(
                    ...
                    severity="HIGH",
                    description=f"Deserialization to internal network address {payload}",
                    detection_method="response_content"
                )
        ...
        for protocol in self.PROTOCOL_SCHEMES:
            ...
            for payload in payloads:
                test = DeserializationTest(
                    ...
                    severity="HIGH",
                    description=f"Deserialization using {protocol} protocol",
                    detection_method="response_content"
                )
    ...
    def _test_url_encoding_bypass(self):
        ...
        for name, payload in bypasses:
            ...
            test = DeserializationTest(
                ...
                severity="HIGH",
                description=f"Deserialization bypass using {name}",
                detection_method="response_content"
            )
    ...
    def _test_ip_obfuscation(self):
        ...
        for name, payload in localhost_obfuscations:
            ...
            test = DeserializationTest(
                ...
                severity="MEDIUM",
                description=f"Deserialization using {name} localhost representation",
                detection_method="response_content"
            )
    ...
    def _test_dns_rebinding(self):
        ...
        for domain in rebinding_domains:
            ...
            test = DeserializationTest(
                ...
                severity="HIGH",
                description=f"Deserialization using DNS rebinding domain {domain}",
    ...
    def _test_blind_deserialization(self):
        """Test blind Deserialization detection."""
        print(f"\n{Fore.YELLOW}[*] Testing blind Deserialization...{Style.RESET_ALL}")
        ...
        test = DeserializationTest(
            name="Blind Deserialization - OAST",
            payload=oast_url,
            category="Blind Deserialization",
            severity="MEDIUM",
            description="Blind Deserialization detection via out-of-band callback",
        ...
        # Time-based detection
        self._test_time_based_deserialization()

    def _test_time_based_deserialization(self):
        """Test time-based blind Deserialization."""
        ...
        for endpoint in slow_endpoints:
            ...
            test = DeserializationTest(
                name=f"Time-based Deserialization - {endpoint}",
                payload=endpoint,
                category="Time-based Deserialization",
                severity="LOW",
                description="Blind Deserialization detected via timing analysis",
    ...
    def _test_redirect_chains(self):
        """Test Deserialization via redirect chains."""
        ...
        for payload in redirect_payloads:
            ...
            test = DeserializationTest(
                ...
                severity="MEDIUM",
                description="Deserialization via HTTP redirect chain",
                detection_method="response_content"
            )
    ...
    def _test_crlf_injection(self):
        ...
        for payload in crlf_payloads:
            ...
            test = DeserializationTest(
                ...
                severity="MEDIUM",
                description="Deserialization with CRLF injection",
                detection_method="response_content"
            )
    ...
    def _execute_test(self, test: DeserializationTest) -> Optional[DeserializationFinding]:
        """
        Execute a single Deserialization test.

        Args:
            test: DeserializationTest to execute

        Returns:
            DeserializationFinding if vulnerability found, None otherwise
        """
        try:
    ...
    def _test_response_content(self, test: DeserializationTest) -> Optional[DeserializationFinding]:
        """Test for Deserialization via response content analysis."""
        response = self._make_request(test.payload)

        if response and self._is_deserialization_response(response, test.category):
    ...
    def _test_timing(self, test: DeserializationTest) -> Optional[DeserializationFinding]:
        """Test for Deserialization via timing analysis."""
        start = time.time()
        response = self._make_request(test.payload, silent=True)
        elapsed = time.time() - start

        # Check if significantly slower than baseline
        if self.timing_baseline and elapsed > (self.timing_baseline * 3):
            return DeserializationFinding(
                severity=test.severity,
                title=test.name,
                category=test.category,
                ...
                evidence={
                    ...
                    'actual_time': f"{elapsed:.3f}s",
                    'time_difference': f"{elapsed - self.timing_baseline:.3f}s"
                },
                impact=self._get_impact(test.category)
            )
    ...
    def _test_dns(self, test: DeserializationTest) -> Optional[DeserializationFinding]:
    ...
    def _test_blind(self, test: DeserializationTest) -> Optional[DeserializationFinding]:
        """Test for blind Deserialization (requires manual verification)."""
        response = self._make_request(test.payload, silent=True)

        # Note: Real blind Deserialization detection requires OAST callback verification
        ...
        if response and response.status_code in [200, 301, 302]:
            return DeserializationFinding(
                ...
                category=test.category,
                payload=test.payload,
                description=f"{test.description} (Manual verification required)",
                evidence={
                    'response_code': response.status_code,
                    'note': 'Check OAST collaborator for callback'
                },
                impact=self._get_impact(test.category)
            )
    ...
    def _is_deserialization_response(self, response: requests.Response, category: str) -> bool:
        """
        Check if response indicates Deserialization vulnerability.
        ...
        Returns:
            True if Deserialization detected
        """
        ...
        # Generic Deserialization indicators
        else:
    ...
    def _get_impact(self, category: str) -> str:
        ...
        impacts = {
            ...
            "URL Encoding Bypass": "Can bypass Deserialization filters using encoded payloads",
            "IP Obfuscation": "Can bypass IP blacklists using alternative IP representations",
            "DNS Rebinding": "Can bypass DNS-based Deserialization protections",
            "Blind Deserialization": "Can trigger requests to attacker-controlled servers, potential data exfiltration",
            "Time-based Deserialization": "Can confirm Deserialization via timing differences",
            "Redirect Chain": "Can bypass Deserialization protections via HTTP redirects",
            "CRLF Injection": "Can inject HTTP headers, manipulate responses"
        }
        return impacts.get(category, "Insecure Deserialization vulnerability")

    def _print_findings_summary(self):
        """Print summary of findings."""
        print(f"\n{Fore.RED}[!] Deserialization VULNERABILITIES FOUND:{Style.RESET_ALL}")
    ...
    def get_findings(self) -> List[DeserializationFinding]:
        """Get all findings."""
        return self.findings

    def get_findings_by_severity(self, severity: str) -> List[DeserializationFinding]:
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python deserialization_tester.py <url_with_INJECT_placeholder> [oast_domain]")
        print("Example: python deserialization_tester.py 'http://example.com/fetch?url=INJECT'")
        print("         python deserialization_tester.py 'http://example.com/fetch?url=INJECT' 'burpcollaborator.net'")
        sys.exit(1)

    target_url = sys.argv[1]
    oast_domain = sys.argv[2] if len(sys.argv) > 2 else None

    tester = DeserializationTester(target_url, oast_domain=oast_domain)
    ...
    if findings:
        print(f"\n{Fore.RED}[!] Deserialization vulnerabilities detected!{Style.RESET_ALL}")
