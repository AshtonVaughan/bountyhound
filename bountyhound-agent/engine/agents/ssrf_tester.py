...
class SSRFTester:
    ...
    def _test_timing(self, test: SSRFTest) -> Optional[SSRFFinding]:
        """Test for SSRF via timing analysis."""
        start = time.time()
    ...
    def _test_blind(self, test: SSRFTest) -> Optional[SSRFFinding]:
        """Test for blind SSRF (requires manual verification)."""
        response = self._make_request(test.payload, silent=True)

        # Note: Real blind SSRF detection requires OAST callback verification
