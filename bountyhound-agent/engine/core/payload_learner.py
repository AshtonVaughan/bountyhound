...
class PayloadLearner:
    ...
    def __init__(self):
        ...
        self.ml_optimizer = MLPayloadOptimizer(db=self.db)
        self.payloads = []
    ...
    def get_ml_optimized_payloads(self, vuln_type: str, count: int = 10) -> List[str]:
        """
        Get ML-optimized payloads for a vulnerability type.

        Uses machine learning to generate and score payloads based on historical success patterns.
        ...
        Returns:
            List of optimized payload strings
        """
        return self.ml_optimizer.generate_optimized_payloads(vuln_type, count=count)

    def score_payload(self, payload: str, vuln_type: str) -> float:
        """
        Score a payload's likelihood of success using ML.

        Args:
            payload: Payload string to score
            vuln_type: Vulnerability type

        Returns:
            Confidence score (0.0-1.0)
        """
        return self.ml_optimizer.predict_payload_success(payload, vuln_type)

    def train_ml_model(self) -> Dict:
        """
        Train the ML model on historical payload data.

        Returns:
            Training result dictionary
        """
        return self.ml_optimizer.train_on_historical_data()
