"""Entry point — runs FastAPI + MCP server for Amass."""
...
log = logging.getLogger("amass-claude")

API_PORT = int(os.environ.get("API_PORT", "8192"))
...
async def main() -> None:
    ...
    log.info("Starting amass-claude...")

    if MCP_MODE:
        from mcp_server import AmassMCP
        mcp = AmassMCP(api_base_url=f"http://127.0.0.1:{API_PORT}")
        mcp.run()
    else:
        tasks = [
