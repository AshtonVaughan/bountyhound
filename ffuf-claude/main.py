"""Entry point — runs FastAPI + MCP server for Ffuf."""
...
log = logging.getLogger("ffuf-claude")
...
async def main() -> None:
    ...
    log.info("Starting ffuf-claude...")

    if MCP_MODE:
        from mcp_server import FfufMCP
        mcp = FfufMCP(api_base_url=f"http://127.0.0.1:{API_PORT}")
        mcp.run()
    else:
        tasks = [
