"""Entry point — runs FastAPI + background cleanup for nessus-claude."""
...
log = logging.getLogger("nessus-claude")

API_PORT = int(os.environ.get("API_PORT", "8196"))
...
async def run_api() -> None:
    ...
    log.info(f"nessus-claude API listening on http://127.0.0.1:{API_PORT}")
    await server.serve()
...
async def main() -> None:
    log.info("Starting nessus-claude...")

    if MCP_MODE:
        from mcp_server import NessusMCP
        mcp = NessusMCP(api_base_url=f"http://127.0.0.1:{API_PORT}")
        mcp.run()
    else:
        tasks = [
        ...
        try:
            ...
            log.info("Shutting down nessus-claude...")
            for t in tasks:
                t.cancel()
