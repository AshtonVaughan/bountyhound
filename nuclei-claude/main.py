"""Entry point — runs FastAPI + MCP server for Nuclei."""
...
log = logging.getLogger("nuclei-claude")

API_PORT = int(os.environ.get("API_PORT", "8188"))
...
async def main() -> None:
    ...
    log.info("Starting nuclei-claude...")

    if MCP_MODE:
        # Run as MCP server only (for embedding in unified MCP)
        from mcp_server import NucleiMCP
        mcp = NucleiMCP(api_base_url=f"http://127.0.0.1:{API_PORT}")
        mcp.run()
    else:
        # Run FastAPI + cleanup
        tasks = [
